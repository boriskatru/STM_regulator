...
enum ZIResult_enum {
  ...
  /// Unknown keyword
  ZI_WARNING_INVALID_KEYWORD = 0x4005,
  ...
  ZI_ERROR_INVALID_ARGUMENT = 0x801F,

  /// Attempt to get a write-only node
  ZI_ERROR_WRITEONLY = 0x8020,

  /// Sample loss detected
  ZI_ERROR_SAMPLE_LOSS = 0x8021,

  /// Errors reported by device
  ZI_ERROR_COM_NACK_BASE = 0x9000,
  ...
  ZI_ERROR_COM_NACK_INTERNAL_BASE = 0x9100,
...
enum ZIValueType_enum {
  ...
  /// ZISHFWaveformVectorData type. Use the ZIEvent.value.vectorData pointer to access the data of the event.
  ZI_VALUE_TYPE_SHF_GENERATOR_WAVEFORM_VECTOR_DATA = 69,

  /// ZISHFResultLoggerVectorData type. Use the ZIEvent.value.vectorData pointer to access the data of the event.
  ZI_VALUE_TYPE_SHF_RESULT_LOGGER_VECTOR_DATA = 70,

  /// ZISHFScopeOutputVectorData type. Use the ZIEvent.value.vectorData pointer to access the data of the event.
  ZI_VALUE_TYPE_SHF_SCOPE_VECTOR_DATA = 71,

  /// ZISHFDemodulatorVectorData type. Use the ZIEvent.value.vectorData pointer to access the data of the event.
  ZI_VALUE_TYPE_SHF_DEMODULATOR_VECTOR_DATA = 72
...
{
  /// Int16 Non-Interleaved, the samples for each enabled scope channel are stored
  /// together one channel after the other,
  // e.g. channel0-sample0, channel0-sample1...channel0-sampleN, channel1-sample0, channel1-sample1...channel1-sampleN
  ZI_SCOPE_SAMPLE_FORMAT_INT16 = 0,
  /// Int32, Non-Interleaved
  ZI_SCOPE_SAMPLE_FORMAT_INT32 = 1,
  /// Float, Non-Interleaved
  ZI_SCOPE_SAMPLE_FORMAT_FLOAT = 2,
  /// Int16 Interleave d, the samples for each enabled scope channel are stored
  /// in successive elements in the data buffer,
  /// e.g. channel0-sample0,channel1-sample0,channel0-sample1,channel1-sample1,channel0-sample2,channel1-sample2.
  ZI_SCOPE_SAMPLE_FORMAT_INT16_INTERLEAVED = 4,
  /// Int32 Interleaved as described for ZI_SCOPE_SAMPLE_FORMAT_INT16INTERLEAVED.
  ZI_SCOPE_SAMPLE_FORMAT_INT32_INTERLEAVED = 5,
  /// Float Interleaved as described for ZI_SCOPE_SAMPLE_FORMAT_INT16INTERLEAVED.
  ZI_SCOPE_SAMPLE_FORMAT_FLOAT_INTERLEAVED = 6,
};
...
enum ZIScopeSampleFormatMask_enum
{
  ...
  ZI_SCOPE_SAMPLE_FORMAT_MASK_DATA_TYPE = 3,
  /// Mask to test the interleaved bit in ZIScopeSampleFormat_enum
  ZI_SCOPE_SAMPLE_FORMAT_MASK_INTERLEAVED = 4
};
...
// --- 8 bytes border
    ...
    ///   - Bit(1): 1 = Perform FFT,
    ///   - Bit(2): 1 = Data originates from a channel-collapsing transform, e.g. |ch0+i*ch1|
    ///   - Bit(7...3): Reserved
...
enum ZIImpFlags_enum {
  ...
  ZI_IMP_FLAGS_ONE_PERIOD = 0x00080000,
  /// One-period measurement values are invalid
  ZI_IMP_FLAGS_ONE_PERIOD_INVALID = 0x00100000,
  /// Reserved for future use.
...
enum ZIVectorElementType_enum {
  ...
  ZI_VECTOR_ELEMENT_TYPE_ASCIIZ = 6,  // NULL-terminated string
  ZI_VECTOR_ELEMENT_TYPE_COMPLEX_FLOAT = 7,
  ZI_VECTOR_ELEMENT_TYPE_COMPLEX_DOUBLE = 8
};

#ifndef __cplusplus
typedef enum ZIVectorElementType_enum ZIVectorElementType_enum;
#endif
...
/// an opaque pointer is used.  See ::ziAPIInit for how to create a ZIConnection.
...
/**
    ...
    Returned names are defined by implementations in the linked library and may change depending on software version.
     ...
     @sa ziAPIConnectEx
 */
DEPRECATED(ZI_EXPORT ZIResult_enum ziAPIListImplementations(char* implementations, uint32_t bufferSize));
...
/**
    ...
    different implementation.
     ...
     @param[in]  implementation  Specifies implementation to use for a connection. NULL selects the default
     ...
     See @link Connection Connection @endlink for an example
     @sa ziAPIConnect, ziAPIDisconnect, ziAPIInit, ziAPIDestroy, ziAPIGetConnectionVersion
 */
...
/**
     ...
     @sa ziAPIConnectEx, ziAPIGetVersion, ziAPIGetCommitHash, ziAPIGetRevision
 */
...
/** Sets the passed pointer to point to the null-terminated release version string of ziAPI.
    ...
    - Other return codes may also be returned, for a detailed error message use ::ziAPIGetLastError.
     ...
     @sa ziAPIConnectEx, ziAPIGetRevision, ziAPIGetCommitHash, ziAPIGetConnectionAPILevel
*/
...
/** Sets the passed pointer to point to the null-terminated commit hash string of ziAPI.
    ...
    @param[in]   commitHash  Pointer to const char pointer.
    @return
    - ZI_INFO_SUCCESS
    - Other return codes may also be returned, for a detailed error message use ::ziAPIGetLastError.

     @sa ziAPIConnectEx, ziAPIGetRevision, ziAPIGetVersion, ziAPIGetConnectionAPILevel
*/
ZI_EXPORT ZIResult_enum ziAPIGetCommitHash(const char** commitHash);
...
/** Sets an unsigned int with the version and build number of the ziAPI you are using.
    ...
    The number is a packed representation of YY.MM.BUILD as a 32-bit unsigned integer:
    (YY << 24) | (MM << 16) | BUILD.

    Note: prior to LabOne 19.10, the packed representation did not contain YY.MM.

    @param[in]   revision  Pointer to an unsigned int to fill up with the packed version number.
    @return
    - ZI_INFO_SUCCESS
    - Other return codes may also be returned, for a detailed error message use ::ziAPIGetLastError.

    @sa ziAPIConnectEx, ziAPIGetVersion, ziAPIGetCommitHash, ziAPIGetConnectionAPILevel
 */
ZI_EXPORT ZIResult_enum ziAPIGetRevision(uint32_t* revision);
...
enum ZIListNodes_enum {
  ...
  ZI_LIST_NODES_EXCLUDESTREAMING = 0x100000,
  /// Exclude node vectors
  ZI_LIST_NODES_EXCLUDEVECTORS = 0x1000000,
  /// @cond deprecated
  // **********
  /// Alias for ZI_LIST_NODES_ALL, no flags specified.
  ZI_LIST_NODES_NONE = 0x00,
  /// Alias for ZI_LIST_NODES_LEAVESONLY.
...
/** This function connects a device with deviceSerial via the specified deviceInterface for use with the server.
    ...
    @param[in]  conn            Pointer to the ZIConnection with which the connection should be established
    @param[in]  deviceSerial    The serial of the device to connect to, e.g., dev1000
...
ZI_EXPORT ZIResult_enum ziAPIGetAuxInSample(ZIConnection conn, const char* path, ZIAuxInSample* value);
  ...
  /**
     ...
     @param[out] length         Pointer to an unsigned int which after the call, contains the length of
                                the retrieved data. If the length of the passed buffer is insufficient,
                                the value is modified to indicate the required minimum buffer size and
                                ZI_ERROR_LENGTH is returned.
...
ZI_EXPORT ZIResult_enum ziAPIGetValueB(ZIConnection conn, const char* path, unsigned char* buffer,
  ...
  /**
     ...
     @param[out] length         Pointer to an unsigned int which after the call, contains the length of
                                the retrieved data (including the null terminator). If the length of the
                                passed buffer is insufficient, the value is modified to indicate the
                                required minimum buffer size and ZI_ERROR_LENGTH is returned.
...
/** This is useful to flush all buffers between API and device to enforce that further code is only executed after
    ...
    deviceSerial parameter is ignored.
     ...
     @param[in]  conn            Pointer to the ZIConnection that is to be synchronized
     @param[in]  deviceSerial    The serial of the device to get the echo from, e.g., dev1000
...
ZI_EXPORT ZIResult_enum ziAPIPollDataEx(ZIConnection conn, ZIEvent* ev, uint32_t timeOutMilliseconds);
  ...
  /**
     ...
     @param[in] conn            Pointer to the ::ZIConnection with which the value should be retrieved
     @param[in] path            Path to the Node holding the value.
                                Note: Wildcards and paths referring to streamimg nodes are not permitted.
...
/** Calling this function enables ziAPI's log at the specified severity level. On Windows the logs can be found by
    ...
    @param[in]  severity        An integer specifying the log's severity level:
                                - trace:   0,
                                - debug:   1,
                                - info:    2,
                                - status:  3,
 ...
 */
ZI_EXPORT void ziAPISetDebugLevel(int32_t severity);

/// Write a message to ziAPI's log with the specified severity.
/** This function may be used to write a message to ziAPI's log from client code to assist with debugging. Note,
 logging must be first enabled using ::ziAPISetDebugLevel.

    @param[in]  severity        An integer specifying the severity of the message to write in the log:
                                - trace:   0,
                                - debug:   1,
                                - info:    2,
                                - status:  3,
...
*/
ZI_EXPORT void ziAPIWriteDebugLog(int32_t severity, const char* message);
...
/** This function is used to configure (set) module parameters which have double types.
    ...
    @param[in]  path            Path of the module parameter to set.
...
/** This function is used to configure (set) module parameters which have vector types.
    ...
    @param[in]  handle          The ::ZIModuleHandle specifying the module to set data on.
    @param[in]  path            Path of the module parameter to set.
    @param[in]  vectorData      Pointer to the vector data.
    @param[in]  elementType     Type of elements stored in the vector.
    @param[in]  numElements     Number of elements of the vector.
    ...
    @sa ziAPIModSetDoubleData, ziAPIModSetIntegerData, ziAPIModSetByteArray
 */
ZI_EXPORT ZIResult_enum ziAPIModSetVector(ZIConnection conn, ZIModuleHandle handle, const char* path, const void* vectorData,
                                          ZIVectorElementType_enum elementType, unsigned int numElements);
...
/** This function is used to retrieve module parameter values of type integer.
    ...
    @param[in]  conn            Pointer to ZIConnection with which the value should be retrieved.
    @param[in]  handle          The ::ZIModuleHandle specifying the module to get the value from.
    @param[in]  path            The path of the module parameter to get data from.
    @param[out] value           Pointer to an 64bit integer in which the value should be written
    @return
    - ZI_INFO_SUCCESS           on success
    - ZI_ERROR_CONNECTION       when the connection is invalid (not connected) or when a communication error occurred
...
/** This function is used to retrieve module parameter values of type vector.
    ...
    @param[in]     handle       The ::ZIModuleHandle specifying the module to get the value from.
    @param[in]     path         The path of the module parameter to get data from.
    @param[out]    buffer       Pointer to a buffer to store the retrieved vector buffer.
    @param[in,out] bufferSize   Pointer to an unsigned int indicating the length of the buffer. If the length of the
                                passed buffer is insufficient to store the vector, the value is modified to indicate
                                the required minimum buffer size and ZI_ERROR_LENGTH is returned.
    @param[out]    elementType  Pointer to store the type of vector elements.
    @param[out]    numElements  Pointer to an unsigned int to store the number of elements of the vector. If the length
                                of the passed buffer is insufficient, a zero will be returned.
...
@sa ziAPIModGetInteger, ziApiModGetDouble, ziAPIModGetString
*/
ZI_EXPORT ZIResult_enum ziAPIModGetVector(ZIConnection conn, ZIModuleHandle handle, const char *path, void* buffer,
                                          unsigned int *bufferSize, ZIVectorElementType_enum *elementType,
...
/** This function returns a list of node names found at the specified path, formatted as JSON. The path may
    contain wildcards so that the returned nodes do not necessarily have to have the same parents.
    The list is returned in a null-terminated char-buffer. If the maximum length of the buffer (bufferSize) is
    not sufficient for all elements, nothing will be returned and the return value will be ::ZI_ERROR_LENGTH.
 ...
 */
ZI_EXPORT ZIResult_enum ziAPIModListNodesJSON(ZIConnection conn, ZIModuleHandle handle, const char* path, char* nodes,
                                              uint32_t bufferSize, uint32_t flags);
...
/** This function terminates the module's thread, releases memory and resources.
    After calling ziAPIModClear the module's handle may not be used any
    more. A new instance of the module must be initialized if required.
    This command is especially important if modules are created repetitively
    inside a while or for loop, in order to prevent excessive memory and resource consumption.
 ...
 *  @brief Functions for working with vector data.
 *  @{
 */

/// vectorElementType - see ::ZIVectorElementType_enum
ZI_EXPORT ZIResult_enum ziAPISetVector(ZIConnection conn, const char* path, const void* vectorData,
                                       uint8_t vectorElementType, uint64_t vectorSizeElements);

/** @} */  // end of group Vector

/** @defgroup Transactions Transactional commands
 *  @brief Functions for handling transactional set commands.
 *  @{
 */

/// Begin a transaction.
ZI_EXPORT ZIResult_enum ziAPIBeginTransaction(ZIConnection conn);

/// Ends a transaction.
ZI_EXPORT ZIResult_enum ziAPIEndTransaction(ZIConnection conn);

/// @sa ziAPISetValueI
ZI_EXPORT ZIResult_enum ziAPITransactionalSetInt(ZIConnection conn, const char* path, ZIIntegerData value);

/// @sa ziAPISetValueD
ZI_EXPORT ZIResult_enum ziAPITransactionalSetDouble(ZIConnection conn, const char* path, ZIDoubleData value);

/// @sa ziAPISetComplexData
ZI_EXPORT ZIResult_enum ziAPITransactionalSetComplex(ZIConnection conn, const char* path, ZIDoubleData real, ZIDoubleData imag);

/// @sa ziAPISetValueB
ZI_EXPORT ZIResult_enum ziAPITransactionalSetBytes(ZIConnection conn, const char* path, unsigned char* buffer, unsigned int length);

/// @sa ziAPISetVector
ZI_EXPORT ZIResult_enum ziAPITransactionalSetVector(ZIConnection conn, const char* path, const void* vectorData,
                                                    uint8_t vectorElementType, uint64_t vectorSizeElements);

/** @} */  // end of group Transactions

/** @defgroup Sequencer Sequencer commands
 *  @brief Functions for working with SeqC programs and waveforms.
 *  @{
 */
...
struct ZICompileResult {
  ...
  /// Size of ELF data
  unsigned int size;
  /// Zero-terminated JSON string with extra information. The "message" element
  /// contains the compiler output.
  char* extra;
};

#ifndef __cplusplus
typedef struct ZICompileResult ZICompileResult;
#endif

/// Allocates ::ZICompileResult structure and returns a pointer to it.
/** This function allocates a ::ZICompileResult structure and returns a pointer to
    it. The memory must be freed using ::ziAPIDeallocateCompileResult.

    @sa ziAPIDeallocateCompileResult
*/
ZI_EXPORT ZICompileResult* ziAPIAllocateCompileResult(void);

/// Deallocates ::ZICompileResult structure created with ::ziAPIAllocateCompileResult.
/** This function is the complement to ::ziAPIAllocateCompileResult.

    @param[in] result Pointer to ::ZICompileResult structure to be deallocated.

    @sa ziAPIAllocateCompileResult
*/
ZI_EXPORT void ziAPIDeallocateCompileResult(ZICompileResult* result);

/// Compile a SeqC program for a sequencer.
/** Compile the sequencer program and return the ELF data for upload to an
    instrument.

    @param[in]  source            SeqC input
    @param[in]  deviceType        Target device type, e.g., HDAWG8, SHFQC
    @param[in]  deviceOptions     List of device options, separated by newlines
                                  as returned by node `/dev.../features/options`.
    @param[in]  coreIndex         Index of the AWG core
    @param[in]  kwargs            JSON object with keyword arguments.
    @param[out] result            Structure to populate with compiler result.
                                  Holds the ELF data and extra output as JSON.

    @return
     - ZI_INFO_SUCCESS            on success
     - ZI_ERROR_GENERAL           if an error occurred during compilation
     - ZI_ERROR_INVALID_ARGUMENT  if an invalid argument was provided
     - In case of error, the "message" property in the extra output contains the
       precise error reason.

    Keyword arguments (JSON):
    - `samplerate` (float): target sample rate of the sequencer.
      Mandatory and only respected for HDAWG. Should match the value set on the device:
      `/dev.../system/clocks/sampleclock/freq`.
    - `sequencer` (str): one of `qa`, `sg`, or `auto`. Mandatory for SHFQC.
    - `wavepath` (str): path to directory with waveforms.
      Defaults to path used by LabOne UI or AWG Module.
    - `waveforms` (str | null): list of CSV waveform files separated by `;`.
       Defaults to an empty list. Set to `null` to include all CSV files in `wavepath`.
      Defaults to all CSV files in `wavepath`.
    - `filename` (str): name of embedded ELF filename.

    @sa ziAPIAllocateCompileResult, ziAPIDeallocateCompileResult,
        ZICompileResult
 */
ZI_EXPORT ZIResult_enum ziAPICompileSeqC(const char* source, const char* deviceType, const char* deviceOptions,
                                         unsigned int coreIndex, const char *kwargs, ZICompileResult* result);

/** @} */  // end of group Sequencer
...
/** Perform a Discovery property look-up for the specified deviceAddress and return its device ID. Attention! This
    ...
    @param[out] deviceIds     Pointer to a buffer that is to contain the list of newline-separated
                              IDs of the devices found, e.g. "dev1000\ndev2000\n".
...
/** Perform a Discovery property look-up for the specified deviceAddress and return its device ID. Attention! This
    ...
    @param[in]  conn          Pointer to ::ZIConnection with which the value should be retrieved.
    @param[in]  deviceAddress The address or ID of the device to find, e.g., 'uhf-dev1000' or 'dev1000'.
    @param[out] deviceId      The ID of the device that was found, e.g. 'DEV1000'.
...
/** Returns the device Discovery properties for a given device ID in JSON format. The function ::ziAPIDiscoveryFind must
    ...
    @param[in]  deviceId      The ID of the device to get Discovery information for, as returned by
                              ::ziAPIDiscoveryFind, e.g., 'dev1000'.
    @param[out] propsJSON     The Discovery properties in JSON format of the specified device.
...
/**
     ...
     hierarchy, e.g., /dev1000/clockbase.
